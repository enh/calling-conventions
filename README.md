...
On arm32, the lr register (and the condition codes) are also clobbered, but on the other architectures only the register used for the return value is clobbered. But note that the x86-64 `syscall` instruction itself clobbers rcx. (See the kernel's [arch-arm.h](https://github.com/torvalds/linux/blob/master/tools/include/nolibc/arch-arm.h) et cetera for more details.)
